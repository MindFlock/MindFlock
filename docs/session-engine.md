...
**Launcher** — provisioned sessions run a generated `.mindflock_launch.sh`:

- exports each cache's env var (`export TESTMON_ENV=shared` etc.; generic —
  one export per `[[workspace.cache]].env` entry)
- first launch seeds the agent with `claude "$(cat .mindflock_prompt.md)"`;
  a `.mindflock_started` marker makes
  later launches resume with `--continue`
- adds `--dangerously-skip-permissions` when `[mindflock].skip_permissions` (default
  on — every worktree is a new path and would otherwise trust-prompt each time)
- exit codes 0/130 drop to a shell; any other exit auto-resumes after 3 s
...
- `provisioned.write_launcher` still exists, but the runtime path goes through
  `providers.resolve("claude").write_launcher(...)`.
