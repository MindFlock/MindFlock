...
from backend.providers.base import LauncherSpec
...
# Prompt launcher
#
# Every CLI-specific spelling in the generated script comes from the launching
# provider's LauncherSpec, via the shared backend.providers.launch_script
# vocabulary — the same helpers the standalone tmux launcher uses, so the two
# ingestion paths cannot disagree about how a CLI is started. The private
# aliases below keep this module's long-standing internal names.
# ---------------------------------------------------------------------------
_apply_binary_override = launch_script.apply_binary_override
_launcher_spec_for = launch_script.spec_for
_launch_program = launch_script.launch_program
_SEED_FN = launch_script.SEED_FN
...
def write_launcher(
    ...
    Provider-neutral. Every CLI-specific spelling in the generated script — the
    skip-permissions flag, how the seed prompt is passed, the resume flag, which
    exit codes count as a deliberate quit — comes from the
    :class:`~backend.providers.base.LauncherSpec` of whichever provider claims
    ``program`` (``spec`` overrides the lookup; used by tests). This is what
    lets an *ingested* session run codex/aider/goose: the script used to hardcode
    Claude Code's four spellings, so a provisioned session on any other CLI was
    started with flags that CLI rejects. For ``program="claude"`` the spec
    reproduces those spellings exactly, so the bytes are unchanged.
    ...
    *re*-launch (where the prior conversation should be resumed instead). When
    ...
    # Which CLI's flags this script speaks. Resolved from ``program`` so an
    # ingested ticket on codex/aider/goose is launched the way that CLI expects;
    # for claude the spec IS the vocabulary this function used to hardcode, so
    # the generated bytes are unchanged.
    spec = spec if spec is not None else _launcher_spec_for(program)
    # The CLI's own skip-all-prompts flag, when the workspace asked for one and
    # the CLI has one at all. Claude's --dangerously-skip-permissions also
    # suppresses its per-folder "Do you trust the files in this folder?" gate,
    # which would otherwise prompt on every freshly-created worktree path.
    flag = (
        f" {spec.skip_perms_flag}"
        if (skip_permissions and spec.skip_perms_flag)
        else ""
    )

    # The seed prompt always lands on disk (so it survives restarts and stays
    # readable when a seed attempt fails). HOW it reaches the agent depends on
    # the CLI: as a launch argument when it accepts one, otherwise typed into
    # the pane once its TUI has drawn.
    seed = ""
    seed_by_keys = False
    if prompt:
        (directory / PROMPT_BASENAME).write_text(prompt, encoding="utf-8")
        if spec.prompt_arg:
            seed = " " + spec.prompt_arg.format(prompt=f'"$(cat {PROMPT_BASENAME})"')
        else:
            seed_by_keys = True

    # Local-model routing (Ollama / LM Studio / any OpenAI-compatible server):
    # its env joins the exports below and its flags ride on the launch program,
    # so every relaunch in the resume chain also stays on the local server.
    # Empty when the feature is off, leaving the script unchanged.
    local_env, local_args = launch_script.local_overlay(program)

    # Cache env exports (default: the shared testmon key when no cache config
    # is in play), plus any local-model env.
    if cache_env is None:
        cache_env = {"TESTMON_ENV": TESTMON_ENV_NAME}
    exports = "".join(
        "export {}={}\n".format(k, _sh_quote(str(v)))
        for k, v in sorted({**cache_env, **local_env}.items())
    )

    # The real executable: the provider's own entry command when it has one, else
    # ``program`` with a user binary-path override (settings/env) swapped in.
    # With neither, this leaves ``program`` untouched, so the generated script is
    # byte-identical to the pre-override behaviour.
    launch_program = _launch_program(program, spec)
    all_args = tuple(local_args) + tuple(launch_args or ())
    if all_args:
        import shlex

        launch_program = "%s %s" % (
            launch_program,
            " ".join(shlex.quote(str(a)) for a in all_args),
        )
    prog = f"{launch_program}{flag}"
    first = f"{prog}{seed}"
    seed_fn = ""
    if seed_by_keys:
        # `seeder & cli` backgrounds the seeder and runs the CLI in the
        # foreground, so `$?` below is still the CLI's exit code.
        seed_fn = launch_script.seed_by_keys_function(PROMPT_BASENAME)
        first = f"{_SEED_FN} & {prog}"
    resume = _resume_chain(prog, spec)
    # Exit codes that mean "the human quit on purpose" -> drop to a shell rather
    # than relaunch. The provider owns the policy (claude/most CLIs: 0 exit,
    # 130 Ctrl-C).
    natural = "|".join(str(int(c)) for c in (spec.natural_codes or (0, 130)))
    ...
    inner = (
        ...
        "while true; do\n"
        "  cs_code=$?\n"
        f'  case "$cs_code" in {natural}) break;; esac\n'
...
def _resume_chain(prog: str, spec: LauncherSpec) -> str:
    """The launcher's resume command: the CLI's own resume flag, one retry after
    a pause, then a PLAIN unseeded launch. The ticket prompt is deliberately NOT
    the fallback: a non-zero exit can't distinguish "nothing to continue" from a
    ...
    Two provider shapes short-circuit the chain. A CLI with no resume flag at all
    (``cline``) can only relaunch fresh. A CLI whose resume flag is harmless with
    nothing to resume (``aider --restore-chat-history`` just starts empty) needs
    no retry/fallback, so it gets the bare resume — a chain there would only add
    two ways to mis-read a real failure.
    """
    if not spec.resume_flag:
        return prog
    resume = f"{prog} {spec.resume_flag}"
    if not spec.resume_fallback:
        return resume
